...
function renderTable(){let rows=snap?.snapshot?.flow?.[active]||[];if(!rows.length){$('table').innerHTML='<div class="empty">Δεν υπάρχουν εγγραφές.</div>';return}let dot=active==='in_production'?'prod':active==='ready'||active==='warehouse'?'ready':'deliv';let head='<table><thead><tr><th>Κατάσταση</th><th>Πελάτης</th><th>Όνομα</th><th>Τύπος</th><th>Διάσταση</th><th>Ημ. Παραγωγής</th><th>Barcode</th><th>Τελευταία ώρα</th></tr></thead><tbody>';let body=rows.map(r=>`<tr><td><span class="dot ${dot}"></span>${active==='in_production'?'Σε παραγωγή':active==='delivered'?'Παραδόθηκε':'Έτοιμο'}</td><td>${esc(r.customer)}</td><td>${esc(r.retail_name)}</td><td>${esc(r.mattress_type||r.product_category)}</td><td>${esc(r.width_cm)} × ${esc(r.length_cm)}</td><td>${esc(r.production_date)}</td><td>${esc(r.unique_code)}</td><td>${esc(lastStatusTime(r))}</td></tr>`).join('');$('table').innerHTML=head+body+'</tbody></table>'}

...
.history{display:none}.history.show{display:block}.historyStatus{border:1px solid var(--line);background:#111827;border-radius:12px;padding:10px 12px;margin-bottom:9px;color:#cbd5e1;font-size:12px}.historyStatus.ok{color:#86efac}.calendarPanel{background:#111827;border:1px solid var(--line);border-radius:14px;padding:12px;margin-bottom:10px}.calHead{display:flex;align-items:center;justify-content:space-between;gap:8px;margin-bottom:8px}.calHead button{width:auto;min-height:38px;padding:7px 12px}.calHead input{width:auto;min-height:38px;background:#0f172a;color:#fff;border:1px solid #334155;border-radius:9px;padding:6px 9px}.calendar{display:grid;grid-template-columns:repeat(7,1fr);gap:5px}.dow{text-align:center;color:#94a3b8;font-size:11px;font-weight:800;padding:3px}.day{min-height:68px;background:#0f172a;border:1px solid #26344d;border-radius:9px;padding:6px;cursor:pointer}.day:hover{border-color:#0ea5e9}.day.blank{background:#0b1220;cursor:default;opacity:.5}.day.today{border:2px solid #0ea5e9}.dayNum{font-size:12px;font-weight:850;margin-bottom:7px}.dayReady{color:#86efac;font-size:11px;font-weight:800}.dayDelivered{color:#7dd3fc;font-size:11px;font-weight:800;margin-top:2px}.historyFilters{display:flex;gap:7px;flex-wrap:wrap;align-items:end;background:#111827;border:1px solid var(--line);border-radius:12px;padding:10px;margin-bottom:9px}.historyFilters label{display:flex;flex-direction:column;gap:4px;color:#94a3b8;font-size:11px}.historyFilters select,.historyFilters input{min-height:38px;background:#0f172a;color:#fff;border:1px solid #334155;border-radius:9px;padding:6px 9px}.historyFilters button{width:auto;min-height:38px;padding:7px 13px}.historyTotals{display:flex;gap:18px;flex-wrap:wrap;margin:3px 3px 9px;font-size:13px;font-weight:850}.historyTotals .r{color:#86efac}.historyTotals .d{color:#7dd3fc}.historyTable .tablewrap{max-height:48vh}.eventReady{color:#86efac;font-weight:800}.eventDelivered{color:#7dd3fc;font-weight:800}
button{border:0;background:#0ea5e9;color:white;font-weight:800;border-radius:10px}@media(max-width:720px){.wrap{padding:13px}.grid{grid-template-columns:repeat(2,1fr)}.value{font-size:26px}.card{padding:13px}.top h1{font-size:19px}.calendar{gap:3px}.day{min-height:60px;padding:4px}.dayReady,.dayDelivered{font-size:9px}.historyFilters{align-items:stretch}.historyFilters label{min-width:46%}}
...
function renderCalendar(data){let counts=data.counts||{},year=Number(data.year),month=Number(data.month);let first=new Date(year,month-1,1),last=new Date(year,month,0),offset=(first.getDay()+6)%7,total=last.getDate();let html=['Δευ','Τρι','Τετ','Πεμ','Παρ','Σαβ','Κυρ'].map(x=>`<div class="dow">${x}</div>`).join('');for(let i=0;i<42;i++){let day=i-offset+1;if(day<1||day>total){html+='<div class="day blank"></div>';continue}let key=`${year}-${String(month).padStart(2,'0')}-${String(day).padStart(2,'0')}`,c=counts[key]||{ready:0,delivered:0},today=key===isoDate(new Date())?' today':'';html+=`<div class="day${today}" data-day="${key}"><div class="dayNum">${day}</div><div class="dayReady">Έτοιμα: ${c.ready||0}</div><div class="dayDelivered">Παραδόθηκαν: ${c.delivered||0}</div></div>`}$('calendar').innerHTML=html;$('calendar').querySelectorAll('.day[data-day]').forEach(el=>el.addEventListener('click',()=>{$('historyMode').value='day';$('historyFrom').value=el.dataset.day;$('historyTo').value=el.dataset.day;refreshHistory(false)}))}
